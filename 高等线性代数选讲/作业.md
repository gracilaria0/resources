...
#### 7.6.4
**设$`n`$阶矩阵$`\bm{A}`$适合$`\bm{A}^2 = \bm{O}`$且$`\bm{A}`$的秩等于$`r`$，试求$`\bm{A}`$的Jordan标准型。**

1. $`r=0`$时，$`\bm{A} = \bm{A}^2 = \bm{O}_n`$，则$`\bm{J} = \bm{O}_n`$。
2. $`r \gt 0`$时，有$`\bm{A} \neq \bm{O}`$。
   
   又$`\bm{A}^2 = \bm{O}`$，所以$`m_{\bm{A}}\left(\lambda\right) = \lambda^2`$，所以$`\bm{A}`$的最后一个不变因子是$`\lambda^2`$。

   根据不变因子间的整除关系，$`\bm{A}`$的不变因子组是$`\overbrace{1, \cdots, 1}^{a_0\text{个}}, \overbrace{\lambda, \cdots, \lambda}^{a_1\text{个}}, \overbrace{\lambda^2, \cdots, \lambda^2}^{a_2\text{个}}`$，其中$$`\left\{\begin{equation*}
		a_0 + a_1 + a_2 &= n, \\
		a_0 &= r, \\
		a_1 + 2a_2 &= n
   \end{equation*}\right.`$$

   所以有$$`\left\{\begin{equation*}
		a_0 &= r, \\
		a_1 &= n - 2r, \\
		a_2 &= r
   \end{equation*}\right.`$$

   分解为准素因子，可得$`\bm{A}`$的初等因子组是$`\overbrace{\lambda, \cdots, \lambda}^{a_1\text{个}}, \overbrace{\lambda^2, \cdots, \lambda^2}^{a_2\text{个}}`$。

   所以$`\bm{J} = \mathrm{diag}\left\{\overbrace{\bm{J}_1\left(0\right), \cdots, \bm{J}_1\left(0\right)}^{a_1\text{个}}, \overbrace{\bm{J}_2\left(0\right), \cdots, \bm{J}_2\left(0\right)}^{a_2\text{个}}\right\} = \mathrm{diag}\left\{\overbrace{0, \cdots, 0}^{n-2r\text{个}}, \overbrace{\bm{J}_2\left(0\right), \cdots, \bm{J}_2\left(0\right)}^{r\text{个}}\right\}`$。
...
#### 7.6.6
**设$`n\left(n \gt 1\right)`$阶矩阵$`\bm{A}`$的秩为$`1`$，试求$`\bm{A}`$的Jordan标准型。**

不满秩的矩阵必有零特征值。

设$`\bm{A}`$的Jordan标准型
$$`\bm{J} = \mathrm{diag}\left\{\bm{J}_{n_1}\left(0\right), \cdots, \bm{J}_{n_k}\left(0\right), \bm{J}_{s_1}\left(\lambda_1\right), \cdots, \bm{J}_{s_m}\left(\lambda_m\right)\right\}`$$

其中，$$`\begin{equation*}
	\mathrm{rank}\bm{J}_{n_i}\left(0\right) &= \mathrm{tr}\left(\begin{matrix}
		0 & 1 \\
		& 0 & 1 \\
		& & \ddots & \ddots \\
		& & & 0 & 1 \\
	...
	\mathrm{rank}\bm{J}_{s_j}\left(\lambda_j\right) &= \mathrm{tr}\left(\begin{matrix}
		\lambda_j & 1 \\
		& \lambda_j & 1 \\
		& & \ddots & \ddots \\
		& & & \lambda_j & 1 \\
	...
	\mathrm{rank}\bm{J} &= \sum_{i=1}^k\left.\mathrm{rank}\bm{J}_{n_i}\left(0\right)\right. + \sum_{j=1}^m\left.\mathrm{rank}\bm{J}_{s_j}\left(\lambda_j\right)\right. &=& \sum_{i=1}^k\left(n_i - 1\right) + \sum_{j=1}^m\left.s_j\right.
\end{equation*}`$$

因为秩是相似不变量，所以$`\mathrm{rank}\bm{J} = \mathrm{rank}\bm{A} = 1`$。

因为$`n_i, s_j \gt 0`$，所以只有两种情况：

1. $`n_i = 0\left(i=1,\cdots,k\right), m=1, s_1=1`$。此时，$`\bm{J} = \mathrm{diag}\left\{0, \cdots, 0, \lambda_1\right\}`$。由于迹是相似不变量，所以$`\lambda_1 = \mathrm{tr}\bm{J} = \mathrm{tr}\bm{A}`$，$`\bm{J} = \mathrm{diag}\left\{0, \cdots, 0, \mathrm{tr}\bm{A}\right\}`$。
2. 有一个$`n_{i_0}=2, n_i=1\left(i\neq i_0\right), m=0`$。此时，$`\bm{J} = \mathrm{diag}\left\{0, \cdots, 0, \bm{J}_2\left(0\right), 0, \cdots, 0\right\} `$。
